...
pub struct CorsConfigs {
    ...
    pub origins: MaybeGlob<HashSet<Origin>>,
...
impl CorsConfigs {
    pub fn origin(&mut self, origin: Origin) -> &mut Self {
        if let MaybeGlob::Value(ref mut origins) = self.origins {
    ...
    pub fn origins(&mut self, origins: HashSet<Origin>) -> &mut Self {
        self.origins = MaybeGlob::Value(origins);
    ...
    // origin == other_origin
    pub fn allows_origin(&self, origin: MaybeGlob<&Origin>) -> bool {
        if origin.is_glob() {
            return true;
        }
    ...
    // pub fn origin_ref(&self, origin: &MaybeGlob<Origin>) -> MaybeGlob<&Origin> {
    //     if origin.is_glob() {
    //         return MaybeGlob::Glob;
    //     }
    //
    //     let Some(Some(origin)) = self.origins.maybe_ref().map(|inner| {
    //         inner
    //             .iter()
    //             .find(|o| MaybeGlob::Value(*o) == origin.as_ref())
    //     }) else {
    //         unreachable!("origin is not allowed by the registered cors")
    //     };
    //
    //     MaybeGlob::Value(origin)
    // }
}
