...
use pheasant_core::{ClientError, ErrorStatus, Header, Headers, ServerError, err_stt};
...
impl Cookies {
    pub fn from_header(set: HashSet<String>) -> Result<Cookies, ErrorStatus> {
        // the collect handles the results inside the iterator
        // if there is an error, the collect would return an Err
        // else it returns an Ok of the unwrapped values collected in a set
        set.into_iter()
            .map(move |mut header| {
            ...
            .collect::<Result<_, ErrorStatus>>()
            .map(|iter| Self { cookies: iter })
    }
}
