...
impl<'a> Respond<'a> {
    pub fn parse(self, stream: &mut impl Write) -> Result<(), std::io::Error> {
        let writer = BufWriter::new(stream);
        let mut n = writer.write(self.proto.as_bytes())?;
        n += writer.write(&[32])?;

        let code = self.status.code().to_string();
        n += writer.write(code.as_bytes())?;
        n += writer.write(&[32])?;
        n += writer.write(self.status.text().as_bytes())?;
        n += writer.write(&[10])?;
        self.headers.write_to(&mut writer)?;
        n += writer.write(&[10])?;
        if !self.body.is_empty() {
            // NOTE body is not sure to be valid utf8
            // since it could have gone through some sort of encoding
            n += writer.write(self.body.as_slice())?;
        }
        writer.flush()?;
