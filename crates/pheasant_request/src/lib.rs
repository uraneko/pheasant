...
use pheasant_middleware::{Cookie, Cookies, Headers, RequestCors};
...
// server
// the server is a container of the endpoints   | got client stream, we only have the server
// - parse stream data into a request           | server contains parsing logic
// - lookup correct endpoint for request        |
//                                              |
// ( endpoints                                  |
//                                              |
//                                              |
// ( resources ))                               | resource contains app logic

// request builder <apply logic using data> ->
// request <apply logic using data> ->
// respond builder <apply logic using data> ->
// respond
...
impl Request {
    ...
    pub fn is_cross_origin(&self) -> bool {
        // WARN not all Options requests are preflight requests
        self.headers.contains("Origin") && self.headers.contains("Access-Control-Request-Method")
    }
