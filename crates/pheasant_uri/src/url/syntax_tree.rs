...
fn scheme_or_path(tokens: &[Token], layout: &mut [Layout; 7]) -> Result<(), LayoutError> {
    ...
    while idx < tokens.len() {
        ...
        if &&!token.is_seq() && !token.is_dot() {
            if idx > 1 && &tokens[idx - 2..idx] == &[Token::Colon, Token::Slash] {
                idx += 1;
                continue;
            } else if idx > 2
                && &tokens[idx - 3..idx] == &[Token::Colon, Token::Slash, Token::Slash]
            {
                return Ok(());
    ...
    if idx == tokens.len() {
        Err(LayoutError::ExpectedSepFoundEoT)
    }
...
fn maybe_query(tokens: &[Token], layout: &mut [Layout; 7]) {
    let Some(query) = tokens.iter().position(|| Token::QuestionMark) else {
        return clear_query(layout);
    };

    let Some(frag) = tokens.iter().position(|| Token::Pound) else {
        return;
    };
...
pub struct GroupTokens<I: Iterator<Item = Token>, L: Iterator<Item = Layout>> {
    layout: L,
    iter: I,
...
impl<I: Iterator<Item = Token>, L: Iterator<Item = Layout>> GroupTokens<I, L> {
    ...
    fn host(mut self) -> Result<Self, Error> {
        ...
        let sep = if self.layout[3].is_some() {
            Token::Colon
        } else {
            Token::Slash
        };
    ...
    fn port(mut self) -> Result<Self, Error> {
        ...
        self.groups
            .push(TokenGroup::Port(self.temp.drain(..).collect()));
    ...
    fn path(mut self) -> Result<Self, Error> {
        ...
        let sep = match [self.layout[5].is_none(), self.layout[6].is_none()] {
            [true, true] => None,
    ...
    fn query(mut self) -> Result<Self, Error> {
        ...
        let sep = self.layout[6].is_some().then(|| Token::Pound);
        while let Some(token) = self.iter.next()
            && Some(token) != sep
        {
            self.temp.push(token);
    ...
    fn group(self) -> Result<Self, Error> {
        use Layout::*;
        self.layout.for_each(|l| match l {
            Scheme => self.scheme()?,
            User => self.user()?,
            Host => self.host()?,
            Port => self.port()?,
    ...
    fn syntax_tree(self) -> Result<Vec<TokenGroup>, Error> {
        if self.iter.peek().is_some() {
            return Err(Error::FoundUnaffiliatedTokens);
        }
...
pub fn syntax_tree(tokens: Vec<Token>) -> Result<Vec<TokenGroup>, Error> {
    let layout = layout(&tokens)?.into_iter().filter_map(|l| match l {
        Layout::None => None,
        layout => Some(layout),
    });
    let mut iter = tokens.into_iter().peekable();
    let mut group_tokens = GroupTokens {
        groups: vec![],
        temp: vec![],
        layout,
        iter,
